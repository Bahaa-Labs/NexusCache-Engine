...
class NexusCacheEngine:
    ...
    def allocate_tokens(self, sequence_id: int, num_tokens: int) -> list[int]:
        return cast(list[int], self.page_table.append_tokens(sequence_id, num_tokens))
    ...
    def prepare_kernel_metadata(
        ...
        block_tables = self.page_table.get_block_table_tensor(
            sequence_ids=sequence_ids, device=device_str
        )
        ...
        return cast(tuple[torch.Tensor, torch.Tensor], (block_tables, slot_mapping))